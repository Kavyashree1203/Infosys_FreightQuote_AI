...
LayerTechnologyUI / App frameworkStreamlitSession tokensPyJWTPublic URL tunnelngrok (via pyngrok)OTP deliveryGmail SMTP (smtplib, Gmail App Password)User storageLocal JSON file (users.json) — sufficient for a Colab demoPassword / answer hashingSHA-256 (hashlib)Secrets managementGoogle Colab Secrets

Project Structure

Milestone1/
├── Milestone1.ipynb     # Main notebook — run this in Google Colab
├── app.py               # Generated by the notebook (Streamlit app source)
├── README.md            # This file
...
Set up the required secrets under Colab Secrets (key icon in the left sidebar), and toggle notebook access on for each:
Secret NameValueJWT_SECRETAny long random string used to sign session tokensNGROK_AUTHTOKENYour personal ngrok AuthtokenEMAIL_PASSWORDA Gmail App Password (requires 2-Step Verification enabled)EMAIL_ADDRESSThe Gmail address that will send OTP emails


Run all cells from top to bottom (Runtime → Run all, or restart the runtime first if re-running).
The last cell prints a public ngrok URL — open it in your browser to use the app.
To reach the Admin Dashboard, open the same URL with ?admin=1 appended, e.g. https://your-ngrok-url/?admin=1.
...
Add screenshots to a screenshots/ folder in this directory and link them below:

PageScreenshotLoginscreenshots/login.pngSignupscreenshots/signup.pngForgot Password — Security Questionscreenshots/forgot-security-question.pngForgot Password — Email OTPscreenshots/forgot-otp.pngOTP Emailscreenshots/otp-email.pngUser Dashboardscreenshots/user-dashboard.pngAdmin Dashboardscreenshots/admin-dashboard.png
